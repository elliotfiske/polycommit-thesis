...
\end{figure}
 ...
 \par Note that we define ``Active Polycommit Users'' as the sample of users with over 5 commitment. 
  ...
  \begin{table}
 \begin{tabular}[h]{ r C{3cm} C{3cm} C{3cm}}
  & \textbf{Non-users} & \textbf{Polycommit \newline Users} & \textbf{Active \newline Polycommit \newline Users} \\
  \hline
  \textbf{All Classes} & & & \\
...
\end{tabular}
\caption{Average midterm scores for different segments of the population. Note that an ``Active User'' is a user with more than 5 commitment.}
\end{table}
